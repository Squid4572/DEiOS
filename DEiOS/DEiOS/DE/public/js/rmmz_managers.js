...
StorageManager.jsonToZip = function(json) {
    return new Promise((resolve, reject) => {
        try {
            const zip = pako.deflate(json, { to: "string", level: 1 });
            const threshold = 100000000; // Adjust this value as needed
            if (zip.length >= threshold) {
...
StorageManager.localFileExists = async function(saveName) {
    try {
        const result = await Filesystem.stat({
            path: this.filePath(saveName),
            directory: Directory.Documents,
        });
        return result !== null;
    } catch (e) {
        // If the file doesn't exist, Filesystem.stat will throw an error
        return false;
    }
};
...
StorageManager.fsMkdir = async function(path) {
    try {
...
StorageManager.fsRename = async function(oldPath, newPath) {
    try {
        ...
        // Read the file from the old path
...
StorageManager.fsUnlink = async function(path) {
    try {
...
StorageManager.fsReadFile = async function(path) {
    try {
        // Check if the file exists
        await Filesystem.stat({
            path: path,
...
StorageManager.fsWriteFile = async function(path, data) {
    try {
        await Filesystem.writeFile({
            path: path,
            data: data,
...
StorageManager.fileDirectoryPath = function() {
    // Use a predefined directory from Capacitor
    const base = Directory.Documents; // or another appropriate directory
    return `${base}/save/`;
};
