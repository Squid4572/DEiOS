...
$._precacheCustomCursors = async function() {
    if (StorageManager.isLocalMode() && Utils.isOptionValid('test')) {
        ...
        if (Utils.RPGMAKER_VERSION >= '1.6.0') {
            path = `${Directory.Documents}/${this.customCursorPath}`;
        } else {
        ...
        try {
            ...
            if (e.message === 'File does not exist') {
                ...
                console.error('Error checking cursor directory:', e);
            }
        }
        // Read dir
        try {
            ...
            console.error('Error reading cursor directory:', e);
        }
    } else {
        // Read stored file
        this._loadAndSetupCustomCursorCSS();
