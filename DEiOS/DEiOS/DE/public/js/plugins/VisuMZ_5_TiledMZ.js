...
// const filesystem = require("/js/native/node_modules/@capacitor/filesystemf/dist/esm/index.js")
...
DataManager.loadMapData = function (mapId) {
    if (this._tempTiledData && this._loadedMapId === mapId) {
...
//////DataManager.loadTiledMapData = async function (mapId) {
//////    this._tiledLoaded = false;
//////    const src = `${mapId}.json`;
//////    const path = `maps/Map${src}`;
//////
//////    try {
//////        // Check if the file exists
//////        await window.Filesystem.stat({
//////            path: path,
//////            directory: window.directory,
//////        });
//////
//////        this._tiledFlag = true;
//////        this._loadedMapId = mapId;
//////
//////        // Read the file
//////        const result = await window.Filesystem.readFile({
//////            path: path,
//////            directory: window.directory,
...
//////            this._tiledFlag = false;
//////            this._tempTiledData = null;
//////        } else {
...
//            status: 200,
//            onload: () => {
//                DataManager.onXhrMapDataLoad(xhr, mapId, src, path);
//            },
//            onerror: () => {
//                DataManager.onXhrError(mapId, src, path);
//            },
//        };
//
//        if (xhr.onload) {
...
DataManager.loadTiledMapData = async function (mapId) {
    ...
    try {
        ...
        if (e.message.includes('no such file')) {
            ...
            console.warn(`Map file ${path} not found. Proceeding without it.`);
            this._tiledFlag = false;
            this._tempTiledData = null;
            this._tiledLoaded = true; // Allow the game to proceed
        } else {
            console.error('Error loading map data', e);
        }
    }
...
DataManager.isMapLoaded = function () {
    return (this._tiledFlag ? (this._tiledLoaded && this._tilesetToLoad <= 0) : true) && VisuMZ.TiledMZ.DataManager_isMapLoaded.call(this);
};
...
Sprite_TiledObject.prototype.setInitialAnimation = function() {
    if (!this._data || this._data._noFrame) return;
...
Game_Map.prototype.checkPassageCollision = function(x, y, bit) {
    ...
    if (collisionMap) {
        ...
        const index = Math.round(x.clamp(0, w - 1)) + w * Math.round(y.clamp(0, h - 1));
