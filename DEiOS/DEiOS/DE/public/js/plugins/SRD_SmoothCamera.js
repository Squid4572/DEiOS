...
Game_Character.prototype.updateScroll = function() {
	if(!$gameMap.isCameraScrolling()) {
		const cut = _.cutOff;
		let xSpeed = Math.floor(this.screenX() + ($gameMap.shiftX * 48) - $gameScreen.zoomX()) * _.power;
		let ySpeed = Math.floor(this.screenY() + ($gameMap.shiftY * 48) - $gameScreen.zoomY()) * _.power;

		xSpeed = Math.trunc(xSpeed * 96) / 96;
		ySpeed = Math.trunc(ySpeed * 96) / 96;
