...
MakerManager.createWindow = function() {
    // Capacitor doesn't support creating new windows like nw.js
    // You might need to handle this differently
    console.log("Creating window for Super Tools Engine - Core Editor | SumRndmDde");
    this.setupWindow();
};
...
MakerManager.moveWindow = function() {
    if (_.move) {
        window.moveBy(-300, 0);
        // Capacitor doesn't support moving windows like nw.js
        console.log("Moving window");
    }
};

MakerManager.setupWindow = function() {
    // Handle window setup differently
...
MakerManager.assignWindow = function() {
    window.MakerManager = this;
    window.DataManagerEX = DataManagerEX;
...
MakerManager.setupGameWindow = function() {
    // Capacitor doesn't support setting up game windows like nw.js
...
FileManager.filePath = function(location) {
	...
	const path = FileManager.filePath = function(location) {
	const base = Directory.Documents;
	return `${base}/${location}`;
};

FileManager.saveData = async function(variable, filename) {
	if (!Utils.isNwjs()) return;
...
FileManager.checkDataExists = async function(filename, info) {
	if (!Utils.isNwjs()) return;
	info = info || "[]";
	if (this.dataPath === undefined) this.dataPath = this.filePath('data/');
	const filePath = `${this.dataPath}${filename}`;
...
FileManager.getFileList = async function(section, folder) {
	if (!Utils.isNwjs()) return [];
	const result = [];
	...
	try {
		...
		for (let i = 0; i < files.files.length; i++) {
			const file = `${location}${files.files[i]}`;
			const stat = await Filesystem.stat({
				path: file,
				directory: Directory.Documents,
			});
			if (!stat.isDirectory && this.isImageFile(files.files[i])) {
				const f = files.files[i].replace('.png', '');
				result.push(f);
			}
		}
	} catch (e) {
...
DebugManager.setToReopen = async function(id) {
	...
	try {
		...
		console.error('Error setting reopen path:', e);
	}
};

DebugManager.removeReopener = async function() {
	const filePath = this.getReopenPath();
	try {
		...
		if (e.message !== 'File does not exist') {
			console.error('Error removing reopen path:', e);
		}
	}
};

DebugManager.isSetToReopen = async function() {
	const filePath = this.getReopenPath();
	try {
		...
		if (e.message === 'File does not exist') {
			...
			console.error('Error checking reopen path:', e);
			return false;
		}
	}
};
