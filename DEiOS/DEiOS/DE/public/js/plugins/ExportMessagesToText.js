        ...
        var getMap = function() {
            ...
            getJSON("data/Map" + num + ".json", function(text) {
                ...
                if (x < window.mapname.length) { // repeat fetching
                    ...
                    // no more maps
                    const isWindowsPath = document.location.pathname.match(/^\/\w:\//) !== null;
                    const basePath = unescape(document.location.pathname.replace(/^\/|index\.html$/g, ''));
                    const fname = isWindowsPath ? `${basePath}${txtname}.txt` : `${basePath}${txtname}.txt`;
                
                    try {
                        await Filesystem.writeFile({
                            path: fname,
                            data: window.tempTextBody,
