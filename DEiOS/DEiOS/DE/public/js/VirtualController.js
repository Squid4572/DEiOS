window.addEventListener('load', function () {
    ...
    mainKeyDiv.style.position = 'absolute'
    mainKeyDiv.style.top = 0
    mainKeyDiv.style.left = 0
    mainKeyDiv.style.width = '100%'
    mainKeyDiv.style.height = '100%'
...
'
    ...
    keySwitch.innerHTML = 'Pad'

    //    如果有作弊模块就显示，没有就不显示
    fetch('cheat/CheatModal.js').then(function(res){
        if(res.status == 404) return
        var cheatButton = document.getElementById('cheat')
        //设置开关大小
        cheatButton.style.width = '50px'
        cheatButton.style.height = '50px'
        //设置开关颜色
        cheatButton.style.background = 'rgba(77, 114, 138, 0.459)'
        // cheatButton.style.display = 'none'
        //设置开关其他属性
        cheatButton.style.position = 'absolute'
        cheatButton.style.right = 0
        cheatButton.style.borderRadius = '50em'
        cheatButton.style.color = 'rgb(255, 255, 255,0.300)'
        cheatButton.style.textAlign = 'center'
        cheatButton.style.lineHeight = cheatButton.style.height
        cheatButton.innerHTML = 'Cheat'
        
        cheatButton.style.zIndex = 2147483647
        
        cheatButton.ontouchstart = function () {
            event.stopPropagation()
            startColor(this)
            vibrate()
            Akeys(this, 50, 'keydown')
        }
        cheatButton.ontouchend = function () {
            endColor(this)
            Akeys(this, 50, 'keyup')
        }
